...
namespace BNG {
    ...
    public class Arrow : MonoBehaviour {
        ...
        private void OnCollisionEnter(Collision collision) {
            ...
            // YR UPDATE
            // start
            // Detect with one of Archery Target was hitted
            if (colNameLower.Contains("target"))
            {
                ArcheryTarget target = collision.gameObject.GetComponent<ArcheryTarget>();
                int targetId = target.id;
                int targedId2 = target.GetId();
                Debug.Log("Archery Target id: " + targetId);
                Debug.Log("Archery Target id: " + targedId2);
            }
            //End
