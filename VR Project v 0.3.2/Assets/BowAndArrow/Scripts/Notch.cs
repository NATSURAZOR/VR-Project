...
public class Notch : XRSocketInteractor
{
    ...
    public bool CanRelese => PullMeasurer.PullAmount > releaseThreshold;
    ...
    {
        // Ñheck for the hover here too, since it factors in the recycle time of the socket
        // Ôlso check that notch is ready, which is set once the bow is picked up
        return QuickSelect(interactable) && CanHover(interactable) && interactable is Arrow && Bow.isSelected;
        // return QuickSelect(interactable) && CanHover(interactable) && interactable is Arrow && Bow.isSelected && base.CanSelect(interactable);
    }
